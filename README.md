 ...
 * 烧开水算法自带3分钟除氯，风冷加速冷却，以及自适应沸点检测功能  
   具体数据需要根据硬件实测，请看tick_water_tank.c
 ...
 * 0x56 写入指定寄存器  
   * 0x00 清除错误标记  
   * 0x01 设置即热温度 0为冷水 恒温： 36 + (n-1) * 0.5度 最高95度  
   * 0x02 开关出水 0xA0 为关闭 0xA1 - 0xA6 为不同水流大小  
   * 0x03 设置出水水量低8位  
   * 0x04 设置出水水量高8位  
  V1.1:  
   * 0x05 初始化水箱(任何非0xBB的值初始化为加热，否则为保温)  
   * 0x06 设置保温温度 0为冷水 恒温： 36 + (n-1) * 0.5度 最高80度  
   * 0x07 开关风冷

关于T16格式：int16_t一位伪小数，114=11.4摄氏度  
关于水流：uint16_t 每单位为1/6mL  
